...
from utils.data_processor import load_dataframe, process_data, process_gerfac_data
from utils.database import save_to_db, save_gerfac_to_db
...
def render_pagina_upload(analise_ativa: str = "retrabalho") -> bool:
    ...
    with col:
        if analise_ativa == "gerfac":
            ...
            uploaded_file = st.file_uploader(
                "Envie o arquivo (.xlsx, .xls, .csv ou .xltx)",
                type=["xlsx", "xls", "csv", "xltx"],
                label_visibility="collapsed",
            )
            st.caption(
                "Colunas esperadas: **INDICADORES**, **Inf. Complementar (P. Serviço)**, "
                "**Dt. Problema** e **Prest. Serviço**. O app reconhece variações de cabeçalho."
            )
            _md("</div>")
        else:
            # Versão Ocorrências de Retrabalho (padrão)
            _md(
                """
                <div style="text-align:center; margin:3rem 0 2rem;">
                    <div class="app-title" style="justify-content:center;">
    ...
    if uploaded_file is not None:
        try:
            with st.spinner("Processando planilha..."):
                ...
                if analise_ativa == "gerfac":
                    df_processado = process_gerfac_data(raw_df)
                else:
        ...
        if analise_ativa == "gerfac":
            save_gerfac_to_db(df_processado)
        else:
